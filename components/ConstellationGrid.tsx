...
interface ConstellationGridProps {
  /** Multiplier on node spacing. 1 = default density, 1.6 = sparser/lighter. */
  density?: number;
}
...
export default function ConstellationGrid({
  ...
  useEffect(() => {
    ...
    const NODE_RGB = "160, 171, 204"; // muted
    const ACCENT_RGB = "0, 238, 255"; // cyan

    const spacing = (isMobile ? 80 : 55) * density;
    const MAX_CONN_DIST = (isMobile ? 60 : 75) * density;
    ...
    // important since multiple ConstellationGrid instances may exist on one page.
